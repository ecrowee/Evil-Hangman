...
Status avl_tree_push_back(TREE hMy_tree, MY_STRING key, MY_STRING item)
{
    ...
    {
        value = my_string_compare((pMy_tree->root)->key, key);

        if (value == 1)
        {
            avl_tree_push_back(&((pMy_tree->root)->right), key, item);
        }
        else if (value == 0)
        {
        ...
        {
            avl_tree_push_back(&((pMy_tree->root)->left), key, item);
        }
        else
        {
            return FAILURE;
...
GENERIC_VECTOR avl_tree_get_largest_family(TREE hMy_tree, int print_val)
{
    ...
    vector = generic_vector_init_default(my_string_init_copy, my_string_destroy);

    for (i = 0; i < generic_vector_get_size(temp); i++)
    {
        generic_vector_push_back(vector, generic_vector_at(temp, i));
    }
